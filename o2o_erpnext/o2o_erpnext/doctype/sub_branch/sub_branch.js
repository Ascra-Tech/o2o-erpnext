frappe.ui.form.on('Sub Branch', {
    refresh: function(frm) {
        ...
        if (frm.doc.custom_supplier) {
            frm.add_custom_button(__('Update'), function() {
                ...
                // Find addresses linked to this supplier and this sub branch
                frappe.call({
                    ...
                    callback: function(r) {
                        if (r.message && r.message.length) {
                            ...
                            // Now check which ones are linked to this specific sub branch
                                    ...
                                    frappe.call({
                                        ...
                                        callback: function(result) {
                                            if (result.message && result.message.links) {
                                                // Check if this address is linked to this sub branch
                                                let linked_to_sub_branch = false;
                                                
                                                for (let link of result.message.links) {
                                                    if (link.link_doctype === 'Sub Branch' && link.link_name === frm.doc.name) {
                                                        linked_to_sub_branch = true;
                                                        break;
                                                    }
                                                }
                                                
                                                // Add a flag to indicate if it's linked to this sub branch
                                                addr.linked_to_sub_branch = linked_to_sub_branch;
                                                resolve(addr);
                                            } else {
                                                addr.linked_to_sub_branch = false;
                                                resolve(addr);
                                            }
                                        }
                                    });
                            ...
                            Promise.all(address_promises).then(addresses => {
                                ...
                                if (billing_addresses.length) {
                                    ...
                                    let billing_options = billing_addresses.map(addr => {
                                        ...
                                        if (addr.linked_to_sub_branch) {
                                            addr_display += ` (${__('Already linked to this Sub Branch')})`;
                                        }
                                        return {
                                            value: addr.name,
                                            label: addr_display
                                ...
                                let d = new frappe.ui.Dialog({
                                    ...
                                    primary_action: function() {
                                        ...
                                        // First, update the sub branch document
                                        frappe.model.set_value(frm.doctype, frm.docname, updates);
                                        
                                        // Then, fetch address details and update sub branch address details fields
                                            ...
                                            return frappe.call({
                                                ...
                                                callback: function(result) {
                                                    if (result.message) {
                                                        ...
                                                        // Also ensure the address has a link to the sub branch
                                                        let has_link = false;
                                                        if (addr.links) {
                                                        ...
                                                        if (!has_link) {
                                                            // Add link to sub branch if not exists
                                                            frappe.call({
    ...
    // Add new function to update tax category based on state
