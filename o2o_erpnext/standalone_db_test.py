...
def investigate_purchase_requisitions_table():
    """Investigate the purchase_requisitions table structure and data"""
    print("\n" + "=" * 60)
    print("INVESTIGATING PURCHASE_REQUISITIONS TABLE")
    print("=" * 60)
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            if not table_exists:
                print("❌ purchase_requisitions table not found!")
                cursor.execute("SHOW TABLES")
                tables = cursor.fetchall()
            ...
            print("✅ purchase_requisitions table found!")
            ...
            print("Field Name | Type | Null | Default | Extra")
            print("-" * 50)
            for col in columns:
                print(f"{col['Field']} | {col['Type']} | {col['Null']} | {col['Default']} | {col['Extra']}")
                
                if col['Field'].lower() == 'category':
                    category_field = col
                    
                if col['Null'] == 'NO':
                    mandatory_fields.append(col['Field'])
            
            # 3. Focus on category field
            print(f"\n2. CATEGORY FIELD ANALYSIS:")
            if category_field:
                print(f"   Field: {category_field['Field']}")
                print(f"   Type: {category_field['Type']}")
                print(f"   Nullable: {category_field['Null']}")
                print(f"   Default: {category_field['Default']}")
                
                # Check existing category values
                cursor.execute("SELECT DISTINCT category, COUNT(*) as count FROM purchase_requisitions WHERE category IS NOT NULL GROUP BY category ORDER BY count DESC LIMIT 10")
                categories = cursor.fetchall()
                
                if categories:
                    print(f"   Existing values:")
                    for cat in categories:
                        print(f"     {cat['category']} (count: {cat['count']}, type: {type(cat['category'])})")
                else:
            ...
            print(f"\n3. MANDATORY FIELDS ({len(mandatory_fields)} total):")
            for field in mandatory_fields:
                print(f"   - {field}")
            
            # 5. Check sample records
            print(f"\n4. SAMPLE RECORDS:")
            cursor.execute("SELECT COUNT(*) as total FROM purchase_requisitions")
            total_count = cursor.fetchone()['total']
            print(f"   Total records: {total_count}")
            
            if total_count > 0:
                cursor.execute("SELECT * FROM purchase_requisitions ORDER BY id DESC LIMIT 2")
            ...
            print(f"\n5. TESTING MINIMAL INSERT:")
            test_data = {
                'entity': 1,
                'subentity_id': 1,
                'order_name': 'Test Order',
                'order_code': 'TEST-001',
                'category': 1,  # Integer value
            ...
            try:
                ...
                cursor.execute("ROLLBACK")  # Don't actually save
                
                print("   ✅ Test insert successful - field types are correct!")
            except Exception as e:
...
if __name__ == "__main__":
    print("Standalone Database Connection Test & Investigation")
    ...
    if success:
        ...
        investigate_purchase_requisitions_table()
